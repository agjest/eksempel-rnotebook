...
huxtable(broom::tidy(chisq.test(table(mtcars$am, mtcars$gear))))
```
...
Legger til litt tekst for å prøve ut GitAhead.
Dokumentet er åpnet i en ny gren startet fra GitAhead.
...
Ønsker vi likninger over flere linjer kan vi bruke (ser bare ut til å virke med \$\LaTeX \$)
...
Lagt til for å se at det virker.

## Appendiks B

Et lite appendiks til.
